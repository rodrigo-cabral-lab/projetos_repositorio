function output = turbofan_ideal_misturado(input)
    %% Turbofan Ideal com escoamento misturado com pós combustor
    %% Inputs:
    ...
    %  T_t7: temperatura total apos afterburner [K]
    ...
    %     f: fuel air ratio
    %  f_AB: razao ar combustivel do afterburner
    %   f_0: razao ar combustivel total
    %     S: thrust specific fuel consumption % convertido para mg/sN
    ...
    elseif isfield(input, 'a')
        % caso a seja fornecido
        a = input.a;
        % razao de temperaturas no fan
    ...
    % tau_hAB razao de entalpia do afterburner
    tau_hAB = T_t7/T_0;
    % razão ar combustivel do afterburner
    f_AB = c_p*T_0*(tau_hAB - tau_h*tau_t*tau_M)/h_PR;
    % razão T_9/T_0
    T_9__T_0= tau_hAB/(tau_r*tau_f);
    % mach de saida
    M_9 = sqrt(2*(tau_r*tau_f-1)/(y-1));
    % razão de velocidade de saida e felocidade do som de entrara com
    % afterburner
    V_9__a_0  = sqrt(T_9__T_0)*M_9; 
    % razao ar combustivel total
    f_0 = f/(1+a)+f_AB;
    % tração especifica
