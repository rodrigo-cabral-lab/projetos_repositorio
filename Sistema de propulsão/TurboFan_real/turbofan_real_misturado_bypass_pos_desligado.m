function output = turbofan_real_misturado_bypass_pos_desligado(input)
    %% Turbofan real - escoamento misturado com bypass especificado - ciclo de análise
    % com afterburner desligado
    %% Inputs:
   ...
   %pi_Mmax:
    ...
    A = n_m*(1+f)*tau_h + tau_r*(a-(tau_c-1));
    B = tau_r*a;
    C = n_m*(1+f)*tau_h/(pi_c*pi_b^((y_t-1)*e_t/y_t))
    tau_f_1 = A/(B+C);
    D = tau_f_1^(((y_t-1)/(y_c-1))*(y_c/y_t)*e_t*e_f -1);
    tau_t_2 = A/(B+C^D);
    ...
    tau_f = tau_f_2;
    % 
    pi_f = tau_f^((y_c*e_f)/(y_c-1));
    % eficiencia do fan
    n_f = (pi_f^((y_c-1)/y_c)-1)/(tau_f -1);
    % 
    tau_t = (pi_f/(pi_c*pi_b))^((y_t-1)*e_t/y_t));
    %
