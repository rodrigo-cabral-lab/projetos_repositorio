function output = turbofan_real_misturado_pos(input)
    %% Turbofan real - escoamento misturado com pós combustor - ciclo de análise
    %% Inputs:
   ...
   %pi_Mmax:
    ...
    %
    a = (n_m*(1+f)*tau_h*(1-(pi_f/(pi_c*pi_b))^((y_t-1)*e_t/y_t)) - ...
        tau_c + 1)/(tau_f -1);
    % 
    ...
    phi = @(M,y)(M^2*(1+(y_t-1)*M^2/2)/(1+y*M^2)^2)
    
    phi_6 = phi(M_6,y_6);
    phi_16 = phi(M_16, y_16);
    ...
    % A_16/A_6
    A_16_A_6 = a_*sqrt(T_t16_T_t6)*M_16/(M_6*sqrt(y_c*R_t*((1+(y_c-1)*M_16^2/2)/...
        (1+(y_t-1)*M_6^2/2))/(y_t*R_c)));
    %
    MFP = @(M,y,R)(M*sqrt(y/R)/((1+(y-1)*M^2/2)^(y+1/(2*(y-1)))));
    % 
    pi_Mideal = (1+a_)*sqrt(tau_m)*MFP(M_6,y_t,R_t)/((1+A_16/A_6)*MFP(M_6A,y_6A,R_6A)) 
    %
    ...
    T19_T0 = tau_r*tau_f/(Pt19_P19)^((y_c-1)/y_c);
    % razao de velocidade de saida do van e mach de entrada
    V19_a0 = M_19*sqrt(T19_T0);
    % velocidade na saida do fan
    V_19 = a_0*V19_a0;
    % pizao
    PI = (pi_r*pi_d*pi_c*pi_b*pi_n)^((y_t-1)/y_t);
    % calculo iterativo de tau_t
    tau_t_1 = 1/PI + (tau_r*(tau_f-1)/(2*n_m*(V_19/V_0-1)))^2/(tau_h*(tau_r-1));
    tau_t_2 = tau_t_1^((e_t-1)/e_t)/PI + (tau_r*(tau_f-1)*(1 + (1-e_t)*tau_t_1^(-1/e_t)/(e_t*PI))/(2*n_m*(V_19/V_0-1)))^2/(tau_h*(tau_r -1)); 
    while abs(tau_t_2 - tau_t_1) > 0.0001
        tau_t_1 = tau_t_2;
        tau_t_2 = tau_t_1^((e_t-1)/e_t)/PI + (tau_r*(tau_f-1)*(1 + (1-e_t)*tau_t_1^(-1/e_t)/(e_t*PI))/(2*n_m*(V_19/V_0-1)))^2/(tau_h*(tau_r -1)); 
    end
    tau_t = tau_t_2;
    ...
    n_t = (1-tau_t)/(1-tau_t^(1/e_t));
    % razao otima de bypass
    a = (n_m*(1+f)*tau_h*(1-tau_t)-tau_r*(tau_c-1))/(tau_r*(tau_f-1));
